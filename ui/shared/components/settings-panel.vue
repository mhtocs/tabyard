...
import { MIN_GRAVEYARD_RETENTION_DAYS } from '@/lib/graveyard/store'
import {
...
function onRetentionBlur(event: Event) {
  ...
  void setGraveyardRetentionDays(value)
}
</script>
...
<template>
  <section class="space-y-6 p-6">
    ...
    <template v-else-if="settings">
      <p class="text-sm text-gray-600">schedule and graveyard retention. changes save immediately.</p>
      ...
      <div class="grid max-w-md gap-4">
        ...
        <label class="block text-sm text-gray-700">
          graveyard retention (days)
          <input
            type="number"
            class="mt-1 w-full rounded border border-gray-300 bg-white px-3 py-2 text-sm text-gray-800 focus:border-blue-400 focus:outline-none"
            :min="MIN_GRAVEYARD_RETENTION_DAYS"
            :value="settings.graveyardRetentionDays"
            @change="onRetentionBlur"
          />
          <span class="mt-1 block text-xs text-gray-500">minimum {{ MIN_GRAVEYARD_RETENTION_DAYS }} days</span>
        </label>
      </div>
